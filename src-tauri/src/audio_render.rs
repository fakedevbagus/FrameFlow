...
mod tests {
  ...
  fn builds_ffmpeg_unified_video_audio_graph_arguments_with_image_input() {
    ...
    assert!(values.windows(2).any(|pair| pair == ["-i".to_string(), "/media/cover.png".to_string()]));
    assert!(values.windows(2).any(|pair| pair == ["-i".to_string(), "/media/broll.mp4".to_string()]));
    assert!(values.windows(2).any(|pair| pair == ["-i".to_string(), "/media/Music Track.mp3".to_string()]));
    assert!(values.windows(2).any(|pair| pair == [
      "-filter_complex".to_string(),
      "[0:v:0]null[vout];[2:a:0]anull[aout]".to_string()
    ]));
    assert!(values.windows(2).any(|pair| pair == ["-map".to_string(), "[vout]".to_string()]));
    assert!(values.windows(2).any(|pair| pair == ["-map".to_string(), "[aout]".to_string()]));
    assert!(values.windows(2).any(|pair| pair == ["-t".to_string(), "5".to_string()]));
    assert!(values.windows(2).any(|pair| pair == ["-c:v".to_string(), "libx264".to_string()]));
