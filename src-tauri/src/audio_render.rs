...
fn build_source_audio_volume_filter(
  ...
  let mut expression = format_number(normalized.last().unwrap().volume.clamp(0.0, 1.0));

  for index in (0..normalized.len() - 1).rev() {
    ...
    let duration_ms = next.time_ms - current.time_ms;
    expression = format!(
      "if(lt(t,{}),{}+({})*((t-{})/{}),{})",
      format_seconds(next.time_ms),
      format_number(current_volume),
      format_number(next_volume - current_volume),
...
mod tests {
  use super::{
    ...
    NativeAudioGraphRenderRequest, NativeSourceAudioSegment, NativeSourceAudioVolumeKeyframe,
