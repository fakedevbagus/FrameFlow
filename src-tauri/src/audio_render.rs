...
#[derive(Deserialize, Clone)]
#[serde(rename_all = "camelCase")]
pub struct NativeSourceAudioEq {
  pub enabled: bool,
  pub low_gain_db: f64,
  pub mid_gain_db: f64,
...
fn build_source_audio_eq_filters(eq: Option<&NativeSourceAudioEq>) -> String {
  let Some(eq) = eq else {
    return String::new();
  };
  ...
  let bands = [
    (120_u32, 0.8_f64, eq.low_gain_db),
    (1_000_u32, 1.0_f64, eq.mid_gain_db),
    (8_000_u32, 0.8_f64, eq.high_gain_db),
  ];

  let mut filters = Vec::new();

  for (frequency, q, gain_db) in bands {
    let gain_db = gain_db.clamp(-12.0, 12.0);

    if gain_db.abs() < 0.000001 {
      continue;
    }
...
fn build_source_audio_fade_filters(segment: &ResolvedSourceAudioSegment) -> String {
  ...
  let fade_in_ms = segment.audio_fade_in_ms.min(duration_ms);
  let fade_out_ms = segment
    .audio_fade_out_ms
    .min(duration_ms.saturating_sub(fade_in_ms));
  ...
  if fade_out_ms > 0 {
    filters.push(format!(
      "afade=t=out:st={}:d={}",
      format_seconds(duration_ms.saturating_sub(fade_out_ms)),
      format_seconds(fade_out_ms),
    ));
  }
...
mod tests {
  ...
  #[test]
  fn builds_unified_graph_with_embedded_source_audio_eq() {
    let video_paths = vec![PathBuf::from("/media/video.mp4")];
    let video_media_types = vec!["video".to_string()];
    let source_audio_segments = vec![ResolvedSourceAudioSegment {
      input_index: 0,
    ...
    let args = build_ffmpeg_video_audio_graph_args(
      ...
      "[0:v:0]null[vout]",
      "[vout]",
      "anullsrc=r=48000:cl=stereo,atrim=duration=4.25,asetpts=PTS-STARTPTS[aout]",
      "[aout]",
      4_250,
      1_280,
      720,
      30.0,
      Path::new("/tmp/source-audio-eq.mp4"),
    )
    .unwrap();
  ...
  #[test]
  fn builds_unified_graph_with_embedded_source_audio_compressor() {
    let video_paths = vec![PathBuf::from("/media/video.mp4")];
    let video_media_types = vec!["video".to_string()];
    let source_audio_segments = vec![ResolvedSourceAudioSegment {
      input_index: 0,
    ...
    let args = build_ffmpeg_video_audio_graph_args(
      ...
      "[0:v:0]null[vout]",
      "[vout]",
      "anullsrc=r=48000:cl=stereo,atrim=duration=5.25,asetpts=PTS-STARTPTS[aout]",
      "[aout]",
      5_250,
      1_280,
      720,
      30.0,
      Path::new("/tmp/source-audio-compressor.mp4"),
    )
    .unwrap();
  ...
  #[test]
  fn builds_unified_graph_with_embedded_source_audio_fades() {
    let video_paths = vec![PathBuf::from("/media/video.mp4")];
    let video_media_types = vec!["video".to_string()];
    let source_audio_segments = vec![ResolvedSourceAudioSegment {
      input_index: 0,
    ...
    let args = build_ffmpeg_video_audio_graph_args(
      ...
      720,
      30.0,
      Path::new("/tmp/source-audio-fades.mp4"),
    )
    .unwrap();
