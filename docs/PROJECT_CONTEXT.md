...
Merge SHA:
`86ec5c5e4943a9b282aa97e9631b73fa0d9fb094`

User validation:
- User reported PASS for M3.121.
- PR #136 was refreshed, its head `bf1aa75ebfbf5e34c5f9e3dd9ebc1cb8c4752745` was verified after the Ready-for-Review transition, and it was squash-merged.
- `main` was verified after merge at `86ec5c5e4943a9b282aa97e9631b73fa0d9fb094`.
- No additional lint/test/build/cargo/manual validation claims are inferred beyond the user's PASS.

Audit finding:
- M3.120 enforced source-media bounds for native unified AV source-audio segments.
- The direct `render_single_source_to_mp4` path still passed optional `source_start_ms` / `source_duration_ms` to FFmpeg without checking the requested range against actual source duration.
- The `render_video_segments_to_mp4` path likewise validated segment structure and media type but did not validate file-backed source ranges against actual media duration.

Implementation:
- Added shared checked source-range validation for legacy native video export paths.
- Single-source exports now validate optional source starts/durations against the probed source duration.
- Multi-segment file-backed sources now validate each requested source range against the probed source duration.
- Reused duration probes for repeated media paths in one multi-segment render.
- Preserved black gap segments without source-range probing.
- Added focused native regression coverage for exact boundaries, start-only requests, overruns, and arithmetic overflow.
- No project schema version change.

Invariant / contract:
- A file-backed native video source range must start no later than the source media duration.
- When a duration is supplied, the source range end must be representable and no greater than the actual source media duration.
- Exact source-end boundaries remain valid.
- Invalid ranges are rejected before FFmpeg execution.

Next step:
- Fresh audit from verified `main` for the next concrete persisted/runtime/native invariant.
...
Merge SHA:
`c0b1ee692156a2b7f11cf130ba79f65efad81dd0`

User validation:
- User reported PASS for M3.120.
- PR #135 was refreshed, its head `d312d3b01ac950f7cdaf5ffe956d8f85f49edcc6` was verified after the Ready-for-Review transition, and it was squash-merged.
...
- The project render plan derives source ranges from validated clip timing, while native direct callers could still request an out-of-range source span.
- FFmpeg could otherwise trim beyond EOF instead of enforcing the same source-range invariant as the project domain.
...
- Rejected source-range overflow using checked integer addition.
- Rejected source ranges whose end exceeds the probed media duration.
...
Merge SHA:
`91179b94d6b986e9c687ef768a23277088abfc28`

User validation:
- User reported PASS for M3.119.
- PR #134 was refreshed, its head `5e721295957b2e4eb8245e06ac914857a4c402ec` was verified after the Ready-for-Review transition, and it was squash-merged.
- `main` was verified after merge.
- No additional lint/test/build/cargo/manual validation claims are inferred beyond the user's PASS.

Audit finding:
- `NativeSourceAudioSegment` accepted source-audio volume/pan, fades, volume keyframes, EQ, and compressor metadata directly from the Tauri request.
- The native validator previously checked only duration, input index, and that the referenced input was a video.
- FFmpeg filter generation could then clamp or normalize values that would violate the project-domain persistence contract.
...
- Added fade duration and non-overlap validation against segment duration.
...
- Source start and duration remain unsigned request fields.

Validation:
- M3.119 is recorded as passed solely from the user's explicit PASS; no individual local command result is inferred.

Remaining risks:
- Native source-audio validation still does not verify that `source_start_ms + duration_ms` stays within the actual media duration.
...
Merge SHA:
`51cc63f8a922cb0189c90d87027f845f3a722d35`

User validation:
- User reported PASS for M3.117.
- PR #132 was refreshed, its head `9a07ecd747d84b429ade86d0a476e9b05750248b` was verified after the Ready-for-Review transition, and it was squash-merged.
- `main` was verified after merge at `51cc63f8a922cb0189c90d87027f845f3a722d35`.
- No additional lint/test/build/cargo/manual validation claims are inferred beyond the user's PASS.
...
- Direct export-setting normalization could therefore produce or preserve a native-invalid request even though the normal preset UI currently supplies safe values.

Implementation:
- Reused `MAX_CANVAS_FRAME_RATE = 240` for export frame-rate normalization.
- Invalid or over-limit export FPS now falls back to the validated project canvas frame rate.
- Positive export dimensions are rounded, raised to the native minimum of 2, and canonicalized to even values.
...
Validation:
- M3.117 is recorded as passed solely from the user's explicit PASS; no individual local command result is inferred.
...
Merge SHA:
`95f5268708f0250e3305d318410ccdbe47d54309`

User validation:
- User reported PASS for M3.116.
- PR #131 was refreshed, its head `0adf81c1120562dab395d366140f589dc743621a` was verified, the PR was marked Ready for Review, and it was squash-merged.
- `main` was verified after merge at `95f5268708f0250e3305d318410ccdbe47d54309`.
- No additional lint/test/build/cargo/manual validation claims are inferred beyond the user's PASS.
...
- A project could therefore remain valid in persistence/runtime and fail only when native export started.

Implementation:
- Added strict positive-even integer validation for persisted `canvas.width` and `canvas.height`.
- Tightened `updateCanvasDimensions()` to enforce the same invariant at command time.
- Added regression coverage for the minimum valid boundary and odd dimensions.
- No project schema version change.
...
- Existing frame-rate behavior remains unchanged.

Remaining risks:
- Native export has additional media/FFmpeg constraints outside the project canvas dimension contract.
- M3.116 introduces no new upper canvas-dimension limit beyond the existing native command/FFmpeg behavior.
...
Merge SHA:
`d7c31e4dcd9c0da664fd76c3de673bbd6d2fedcc`
...
- Existing Scale range `0.05` to `10` remains unchanged.
- Added regression coverage for runtime normalization, command behavior, and persisted-value rejection.
- User reported PASS.
- No project schema version change and no change to X/Y, Rotation, Opacity, Crop, Preview, or Export contracts.
...
Merge SHA:
`a66fdc25ad5eebcbdd2a5b70451c4a4ecaacdcea`
...
- Existing non-empty-name validation and project schema version remain unchanged.
- User reported PASS.
- No runtime/editor/preview/export behavior change for canonical names.

Next step:
- M3.108 — Canonical Track Volume/Pan Precision: align track audio-control persistence with the existing 0.01 UI step.
...
Merge SHA:
`2918b809edf39306e15fdf884ce11ae3558fee7f`

Implementation reconciled:
- Persisted Audio Volume Automation keyframe `timeMs` values now require integer milliseconds.
- Persistence aligns with the runtime normalizer, which rounds audio automation timestamps before ordering/deduplication.
...
- RenderPlan carries normalized Video clip audio fade durations for Video assets using the existing `getAudioFadeDurations` helper.
...
- Native source-audio rendering accepts backward-compatible optional fade fields.
- Native FFmpeg applies duration-clamped `afade` filters before timeline delay.
- Added RenderPlan, export-pipeline, and Rust regression coverage.
- Updated project context, changelog, and handoff documentation.

Validation:
- User reported PASS after local validation of M3.79, including the requested embedded Video source-audio fade export checks.
...
Next step:
- In a new session, re-inspect updated `main`, open PRs/branches, and repository documentation before selecting the next focused milestone.

## M3.78 — Embedded Video Audio Preview Consistency — completed — 2026-09-24

Branch:
`feat/m3-78-video-source-audio-preview`

PR:
#92

Merge SHA:
`bd583a3891c9acf68ab827e31469008ce1f8e015`

Scope:
- Make embedded audio played by Video clips honor the same audio controls already used for Audio-track preview/export.
- Reuse the existing project model and audio automation functions.
- Apply Track Volume, Track Pan, clip Volume Automation, and clip Fade In/Fade Out to Video preview.
- Keep Audio-track preview behavior unchanged.

Implementation completed:
- Generalized the existing preview Web Audio routing from Audio elements to HTML media elements.
- Added shared media-audio state application so Audio and Video media use the same effective volume calculation.
- Video preview now applies Track Volume × clip Volume Automation × Fade gain.
- Video preview reuses the existing StereoPanner and optional EQ/compressor processing path so Track Pan remains consistent in preview.
- Re-runs Video audio routing when the prepared preview source URL becomes available.
- Added regression coverage for the combined Video embedded-audio gain path.
- Kept export behavior and the project schema unchanged.

Validation:
- User reported PASS after local validation of the M3.78 embedded Video audio preview scope.

Known limitations:
- Export behavior for embedded Video source audio remains unchanged by this milestone.
- Video source-audio export fade handling remains a separate milestone.
- Video source-audio export EQ/compressor remains separate future work.
- No new project schema fields or Video-specific audio Inspector controls were introduced.

Next step:
- Start M3.79 from updated `main`, focusing on exporting Video-clip embedded source-audio Fade In/Fade Out consistently with preview.
...
Branch:
`feat/m3-72-multitrack-audio-export`

PR #86

Merge SHA:
`8aaae96d09bd7275c9171dc08cf89b74e1dd8e38`

Implemented:
- Removed the obsolete visual graph compiler rejection of explicit Audio track segments.
- Enabled single- and multi-video-track visual graph compilation alongside explicit Audio tracks.
- Reused the M3.71 unified AV native renderer for multi-video + explicit-audio export in one FFmpeg invocation.
- Preserved video-input rebasing and audio-input offsetting.
- Added regression coverage for single-track + audio, multi-track + audio, and unified pipeline routing.
- No project schema change.

Validation:
- User reported PASS for the requested local lint, frontend test suite, production build, Rust test suite, and Tauri development startup.
- The pre-PASS run had one stale graph-label assertion; it was corrected to match the fixture's actual project track index before the successful rerun.

Known limitation:
- Embedded source-audio preservation for graph-rendered visual clips is not yet included in the unified AV export path.

Next step:
- M3.73 should focus narrowly on preserving embedded/source audio from graph-rendered visual clips while explicit Audio tracks are present.
