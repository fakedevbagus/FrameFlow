...
- Persisted canvas width/height now require positive integers at parse time.
- Positive fractional frame rates remain valid.
- Added parser regressions for fractional dimension rejection and valid fractional frame rate.
- User reported PASS.
...
### M3.94 — completed
- PR #109; merge SHA `6048b038e4d15e00372e0267a9e0d2f14caaf72b`.
- Persisted Canvas width/height validation is aligned with the existing canvas mutation contract.
- PR #76 remains parked; PR #22 remains unrelated and untouched.
- The next session must re-inspect actual `main`, docs, open PRs/branches, and relevant source before selecting the next milestone.
